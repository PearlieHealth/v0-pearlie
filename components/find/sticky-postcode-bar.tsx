...
export function StickyPostcodeBar({ heroSearchId = "hero-postcode-search" }: StickyPostcodeBarProps) {
  ...
  return (
    ...
    >
      <div className="rounded-[6vw] bg-[#111218]/80 backdrop-blur-[40px] backdrop-saturate-[1.4] border border-[#1f2133]/60 shadow-[0_4px_24px_rgba(0,0,0,0.06),0_1px_2px_rgba(0,0,0,0.04)] px-4 py-3">
        <form onSubmit={handleSubmit} className="flex gap-2.5 items-center">
          <div className="relative flex-1">
            <input
              ...
              className="w-full h-10 px-4 text-sm rounded-full border border-border/40 bg-card/80 focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary placeholder:text-muted-foreground/60"
            />
            {isValidating && (
