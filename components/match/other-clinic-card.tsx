...
export function OtherClinicCard({ clinic, isSelected, onClick }: OtherClinicCardProps) {
  ...
  return (
    <Card
      className={cn(
        "p-2.5 sm:p-3 cursor-pointer transition-all duration-200 hover:shadow-md active:scale-[0.98]",
        isSelected
            ...
            <div className="flex gap-1 sm:gap-1.5 pt-0.5">
              {clinic.highlight_chips.slice(0, 2).map((chip) => {
                ...
                return (
                  <span
                    key={chip}
                    className="inline-flex items-center gap-1 text-[10px] font-medium text-muted-foreground bg-muted/60 px-1.5 py-0.5 rounded"
                  >
                    {chipData.label}
                  </span>
                )
