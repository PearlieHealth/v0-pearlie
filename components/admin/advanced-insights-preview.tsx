...
export function AdvancedInsightsPreview(_props?: {
  ...
  return (
    <Card className="p-4 md:p-8 bg-gradient-to-br from-secondary to-card border-2 border-[#e5e4df]">
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2 md:gap-3">
            <h3 className="text-lg md:text-xl font-bold text-foreground">Advanced Insights</h3>
            <Badge variant="secondary" className="bg-[#0d1019] text-white hover:bg-[#0d1019]/90">
        ...
        <Button variant="default" className="w-full md:w-auto bg-[#0d1019] hover:bg-[#0d1019]/90">
          Request Access
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
        {proFeatures.map((feature) => {
          ...
          return (
            <div
              key={feature.title}
              className="p-3 md:p-4 bg-card/50 backdrop-blur-sm border border-[#e5e4df] rounded-lg relative overflow-hidden"
            >
              <div className="absolute top-2 right-2">
                <Lock className="h-3 w-3 md:h-4 md:w-4 text-muted-foreground/30" />
              </div>
              <div className="flex items-start gap-2 md:gap-3">
                <div className="p-1.5 md:p-2 bg-secondary rounded-md flex-shrink-0">
                  <Icon className="h-4 w-4 md:h-5 md:w-5 text-foreground" />
                </div>
                <div className="flex-1 min-w-0 pr-4">
                  <h4 className="font-semibold text-xs md:text-sm text-foreground mb-1 break-words">{feature.title}</h4>
