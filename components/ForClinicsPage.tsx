...
function MiniBar({ label, pct, highlight }: { label: string; pct: number; highlight?: boolean }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
      <span style={{ fontSize: 12, color: highlight ? "hsl(var(--foreground))" : "hsl(var(--muted-foreground))", fontWeight: highlight ? 600 : 400, width: 100, flexShrink: 0, textAlign: "right" }}>{label}</span>
      <div style={{ flex: 1, height: 8, borderRadius: 4, background: "hsl(var(--border))", overflow: "hidden" }}>
        <div style={{ width: `${pct}%`, height: "100%", borderRadius: 4, background: highlight ? "linear-gradient(90deg, hsl(var(--primary)), hsl(var(--primary)))" : "hsl(var(--border))", transition: "width .3s" }} />
      </div>
    </div>
  )
}
...
function Tag({ label, active }: { label: string; active?: boolean }) {
  return (
    <span style={{ display: "inline-block", padding: "5px 12px", borderRadius: 8, fontSize: 12, fontWeight: active ? 600 : 400, background: active ? "hsla(var(--primary),.08)" : "hsl(var(--secondary))", color: active ? "hsl(var(--primary))" : "hsl(var(--muted-foreground))", border: `1px solid ${active ? "hsla(var(--primary),.2)" : "hsl(var(--border))"}`, lineHeight: 1.3 }}>
      {label}
    </span>
  )
}
...
function VisualInsightCard({ icon, title, question, children }: { icon: React.ReactNode; title: string; question: string; children: React.ReactNode }) {
  return (
    <div style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 16, padding: "24px 22px 20px", display: "flex", flexDirection: "column", boxShadow: "0 1px 4px rgba(0,0,0,.08)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <div style={{ width: 34, height: 34, borderRadius: 10, background: "hsla(var(--primary),.08)", display: "flex", alignItems: "center", justifyContent: "center", color: "hsl(var(--primary))", flexShrink: 0 }}>{icon}</div>
        <span style={{ fontWeight: 700, color: "hsl(var(--foreground))", fontSize: 14 }}>{title}</span>
      </div>
      <div style={{ flex: 1, marginBottom: 12 }}>{children}</div>
      <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, lineHeight: 1.4, marginTop: "auto" }}>
        From questionnaire: <span style={{ color: "hsl(var(--muted-foreground))" }}>{question}</span>
      </p>
    </div>
  )
}
...
function FaqItem({ question, answer }: FaqItemProps) {
  ...
  return (
    <div style={{ borderBottom: "1px solid hsl(var(--border))", padding: "22px 0" }}>
      <button onClick={() => setOpen(!open)} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", background: "none", border: "none", cursor: "pointer", color: "hsl(var(--foreground))", fontWeight: 600, fontSize: 16, textAlign: "left", padding: 0, fontFamily: "inherit" }}>
        {question}
        <ChevronDown size={18} style={{ transform: open ? "rotate(180deg)" : "rotate(0)", transition: "transform .2s", color: "hsl(var(--muted-foreground))", flexShrink: 0, marginLeft: 16 }} />
      </button>
      {open && <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 15, lineHeight: 1.7, marginTop: 12 }}>{answer}</p>}
    </div>
  )
}
...
export default function ForClinicsPage() {
  ...
  const greenBtn = {
    ...
    boxShadow: "0 0 20px hsla(var(--primary),.15), 0 4px 12px rgba(0,0,0,.08)",
  } as const
  const outlineBtn = {
    display: "inline-flex",
    alignItems: "center",
  ...
  const sectionSub = (text: string, width = 560) => (
    <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 17, textAlign: "center", maxWidth: width, margin: "0 auto 56px", lineHeight: 1.6 }}>{text}</p>
  )

  return (
    <div className="for-clinics" style={{ background: navy, color: "hsl(var(--foreground))", minHeight: "100vh", fontFamily: "var(--font-jakarta), 'Plus Jakarta Sans', sans-serif", WebkitFontSmoothing: "antialiased" }}>
      {/* ── Mobile overrides ── */}
      <style>{`
        input[type="range"] { -webkit-appearance: none; appearance: none; height: 6px; background: hsl(var(--border)); border-radius: 3px; outline: none; cursor: pointer; }
        ...
        input[type="range"]::-moz-range-track { height: 6px; background: hsl(var(--border)); border-radius: 3px; }
        ...
        @media (max-width: 640px) {
          ...
          .fc-mobile-dropdown[data-open="true"] {
            ...
            top: 64px; left: 0; right: 0; background: hsla(var(--card),.98);
            backdrop-filter: blur(16px); border-bottom: 1px solid hsl(var(--border));
            padding: 12px 24px 20px; gap: 0;
          }
          .fc-mobile-dropdown a { display: block; padding: 14px 0; color: hsl(var(--muted-foreground)); text-decoration: none; font-size: 16px; font-weight: 500; border-bottom: 1px solid hsl(var(--border)); }
      ...
      {/* ══════════ 1. NAV ══════════ */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: "hsla(var(--card),.92)", backdropFilter: "blur(12px)", borderBottom: "1px solid hsl(var(--border))", boxShadow: "0 1px 3px rgba(0,0,0,.05)" }}>
        <div style={{ ...maxW, display: "flex", alignItems: "center", justifyContent: "space-between", height: 64, padding: "0 24px" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}><Link href="/" style={{ textDecoration: "none" }}><span style={{ ...heading, fontSize: 22 }}>Pearlie</span></Link><span style={{ fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: "hsl(var(--primary))" }}>Clinics</span></div>
          <div className="fc-nav-links" style={{ display: "flex", alignItems: "center", gap: 28 }}>
            {[{ href: "#how", label: "How it works" }, { href: "#insights", label: "Insights" }, { href: "#pricing", label: "Pricing" }, { href: "#faq", label: "FAQ" }].map((l) => (
              <a key={l.href} href={l.href} style={{ color: "hsl(var(--muted-foreground))", textDecoration: "none", fontSize: 13, fontWeight: 500 }}>{l.label}</a>
            ))}
            <a href="#pricing" style={{ ...greenBtn, padding: "9px 20px", fontSize: 13, borderRadius: 10 }}>Get started</a>
          </div>
          <button className="fc-mobile-menu-btn" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} aria-label="Toggle menu" style={{ background: "none", border: "none", cursor: "pointer", color: "hsl(var(--foreground))", padding: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <section className="fc-hero fc-section" style={{ ...sec, paddingTop: 160, paddingBottom: 100, textAlign: "center" }}>
        <div style={maxW}>
          ...
          <p style={{ color: "hsl(var(--muted-foreground))", fontSize: "clamp(17px, 2vw, 20px)", lineHeight: 1.6, maxWidth: 600, margin: "0 auto 44px" }}>
          ...
          <div className="fc-stats-bar" style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 16, padding: "32px 24px", borderTop: `1px solid ${cardBorder}`, borderBottom: `1px solid ${cardBorder}` }}>
            {["No contracts", "No setup fee", "Cancel anytime"].map((item) => (
              <span key={item} style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "hsla(var(--primary),.07)", border: "1px solid hsla(var(--primary),.2)", padding: "10px 22px", borderRadius: 24, color: "hsl(var(--primary))", fontSize: 15, fontWeight: 700 }}>
                <Check size={16} /> {item}
              </span>
            ))}
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 24, marginTop: 20 }}>
            {[{ icon: <Shield size={13} />, label: "GDPR Compliant" }, { icon: <Lock size={13} />, label: "End-to-End Encrypted" }, { icon: <Server size={13} />, label: "UK Data Hosting" }].map((b) => (
              <div key={b.label} style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "hsl(var(--muted-foreground))", fontSize: 11, fontWeight: 500 }}>
          ...
          <div className="fc-steps-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24 }}>
            ...
            <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, padding: "28px 24px", display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                <div style={{ width: 42, height: 42, borderRadius: 12, background: `hsla(var(--primary),.1)`, display: "flex", alignItems: "center", justifyContent: "center", color: teal }}><FileText size={22} /></div>
                <span style={{ color: teal, fontWeight: 800, fontSize: 13, letterSpacing: "0.04em" }}>STEP 01</span>
              </div>
              <h3 style={{ fontWeight: 700, color: "hsl(var(--foreground))", fontSize: 18, marginBottom: 8 }}>Patient completes intake</h3>
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 14, lineHeight: 1.55, marginBottom: 16 }}>Our questionnaire captures treatment needs, anxiety level, cost approach, concerns, values, and preferred timing.</p>
              <div style={{ background: "rgba(0,0,0,.04)", borderRadius: 12, padding: "16px 18px", flex: 1 }}>
                ...
                <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, marginBottom: 8, fontWeight: 500 }}>What matters most when choosing a clinic?</p>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {[{ label: "Clear pricing before treatment", selected: true }, { label: "A calm, reassuring environment", selected: true }, { label: "Strong reputation and reviews", selected: false }].map((opt) => (
                    <div key={opt.label} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", borderRadius: 8, background: opt.selected ? `hsla(var(--primary),.1)` : "rgba(0,0,0,.03)", border: `1px solid ${opt.selected ? "hsla(var(--primary),.25)" : "rgba(0,0,0,.06)"}` }}>
                      <div style={{ width: 14, height: 14, borderRadius: 4, border: `2px solid ${opt.selected ? teal : "hsl(var(--muted-foreground))"}`, background: opt.selected ? teal : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        {opt.selected && <Check size={9} style={{ color: "#fff" }} />}
                      </div>
                      <span style={{ fontSize: 11, color: opt.selected ? "hsl(var(--foreground))" : "hsl(var(--muted-foreground))" }}>{opt.label}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, padding: "28px 24px", display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                <div style={{ width: 42, height: 42, borderRadius: 12, background: `hsla(var(--primary),.1)`, display: "flex", alignItems: "center", justifyContent: "center", color: teal }}><Brain size={22} /></div>
                <span style={{ color: teal, fontWeight: 800, fontSize: 13, letterSpacing: "0.04em" }}>STEP 02</span>
              </div>
              <h3 style={{ fontWeight: 700, color: "hsl(var(--foreground))", fontSize: 18, marginBottom: 8 }}>Pearlie matches to your clinic</h3>
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 14, lineHeight: 1.55, marginBottom: 16 }}>Our algorithm scores patients on clinical fit, treatment availability, cost alignment, and what they value most.</p>
              <div style={{ background: "rgba(0,0,0,.04)", borderRadius: 12, padding: "16px 18px", flex: 1 }}>
                <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, marginBottom: 12, fontWeight: 500 }}>Match score breakdown</p>
                {[{ label: "Treatment fit", score: 95, color: teal }, { label: "Cost alignment", score: 88, color: tealLight }, { label: "Location", score: 82, color: "#34d399" }, { label: "Clinic values match", score: 90, color: teal }].map((m) => (
                  <div key={m.label} style={{ marginBottom: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ color: "hsl(var(--muted-foreground))", fontSize: 11 }}>{m.label}</span>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 14, paddingTop: 12, borderTop: `1px solid ${cardBorder}` }}>
                  <span style={{ color: "hsl(var(--foreground))", fontSize: 13, fontWeight: 600 }}>Overall match</span>
            ...
            <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, padding: "28px 24px", display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
                <div style={{ width: 42, height: 42, borderRadius: 12, background: `hsla(var(--primary),.1)`, display: "flex", alignItems: "center", justifyContent: "center", color: teal }}><Zap size={22} /></div>
                <span style={{ color: teal, fontWeight: 800, fontSize: 13, letterSpacing: "0.04em" }}>STEP 03</span>
              </div>
              <h3 style={{ fontWeight: 700, color: "hsl(var(--foreground))", fontSize: 18, marginBottom: 8 }}>You receive a matched patient</h3>
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 14, lineHeight: 1.55, marginBottom: 16 }}>Full profile with all 8 data points — anxiety level, blockers, budget mindset, preferred times, and more.</p>
              <div style={{ background: "rgba(0,0,0,.04)", borderRadius: 12, padding: "16px 18px", flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                  ...
                  <div><p style={{ color: "hsl(var(--foreground))", fontSize: 13, fontWeight: 600 }}>Sarah M.</p><p style={{ color: "hsl(var(--muted-foreground))", fontSize: 10 }}>Requested appointment</p></div>
                  <span style={{ marginLeft: "auto", fontSize: 10, fontWeight: 700, color: teal, background: `hsla(var(--primary),.12)`, padding: "3px 8px", borderRadius: 6 }}>92% match</span>
                </div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {["Invisalign", "Quite anxious", "Flexible range", "Within a week", "Clear pricing", "Afternoons", "Up to 5 mi", "Cost concern"].map((l) => (
                    <span key={l} style={{ fontSize: 10, fontWeight: 500, padding: "3px 8px", borderRadius: 6, background: `hsla(var(--primary),.1)`, color: tealLight, border: `1px solid hsla(var(--primary),.2)` }}>{l}</span>
                  ))}
                </div>
              </div>
            </div>
        ...
        <div style={maxW}>
          ...
          <div className="fc-insights-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 16 }}>
            ...
            <VisualInsightCard icon={<PoundSterling size={16} />} title="Cost approach" question="How do you think about investing in treatment?">
              ...
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 12, marginTop: 10, lineHeight: 1.4 }}>Includes monthly payment preference and budget range when shared</p>
            </VisualInsightCard>
            <VisualInsightCard icon={<AlertCircle size={16} />} title="What's holding them back" question="Is there anything making you hesitate?">
              ...
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 12, marginTop: 10, lineHeight: 1.4 }}>Up to 2 selected per patient</p>
            </VisualInsightCard>
            <VisualInsightCard icon={<Star size={16} />} title="What they value in a clinic" question="What matters most when choosing a clinic?">
              {[{ label: "Clear pricing", pct: 65, top: true }, { label: "Calm environment", pct: 50, top: true }, { label: "Specialist experience", pct: 45 }, { label: "Good reviews", pct: 40 }, { label: "Flexible hours", pct: 30 }, { label: "Continuity of care", pct: 25 }].map((v) => (
                <div key={v.label} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 5 }}>
                  <span style={{ fontSize: 12, color: v.top ? "hsl(var(--foreground))" : "hsl(var(--muted-foreground))", fontWeight: v.top ? 600 : 400, width: 120, flexShrink: 0, textAlign: "right" }}>{v.label}</span>
                  <div style={{ flex: 1, height: 8, borderRadius: 4, background: "rgba(0,0,0,.06)", overflow: "hidden" }}>
                    <div style={{ width: `${v.pct}%`, height: "100%", borderRadius: 4, background: v.top ? `linear-gradient(90deg, ${teal}, ${tealLight})` : "hsl(var(--border))" }} />
                  </div>
                </div>
              ))}
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 12, marginTop: 8, lineHeight: 1.4 }}>Each patient picks their top 2 priorities</p>
            </VisualInsightCard>
            <VisualInsightCard icon={<Clock size={16} />} title="How ready they are" question="When are you looking to start treatment?">
              <div style={{ display: "flex", alignItems: "stretch", gap: 0, marginBottom: 8 }}>
                {[{ label: "ASAP", color: teal }, { label: "This week", color: tealLight }, { label: "Few weeks", color: "#34d399" }, { label: "Exploring", color: "hsl(var(--muted-foreground))" }].map((t, i) => (
              ...
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 12, lineHeight: 1.4 }}>Emergency patients show same-day / next-day urgency</p>
            </VisualInsightCard>
            <VisualInsightCard icon={<Stethoscope size={16} />} title="Treatment interest" question="What treatment are you looking for?">
              ...
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 12, marginTop: 10, lineHeight: 1.4 }}>Patients can select multiple treatments</p>
            </VisualInsightCard>
            <VisualInsightCard icon={<Calendar size={16} />} title="Preferred visit times" question="When would you prefer appointments?">
              <div style={{ display: "flex", gap: 10, marginBottom: 8 }}>
                {[{ label: "Morning", sub: "Before 12pm", icon: "9am", active: false }, { label: "Afternoon", sub: "12 – 5pm", icon: "2pm", active: true }, { label: "Weekend", sub: "Sat / Sun", icon: "Sat", active: true }].map((slot) => (
                  <div key={slot.label} style={{ flex: 1, textAlign: "center", padding: "14px 8px 12px", borderRadius: 12, background: slot.active ? `hsla(var(--primary),.1)` : "rgba(0,0,0,.03)", border: `1px solid ${slot.active ? "hsla(var(--primary),.25)" : "rgba(0,0,0,.06)"}` }}>
                    <p style={{ fontSize: 18, fontWeight: 800, color: slot.active ? teal : "hsl(var(--muted-foreground))", marginBottom: 4, fontFamily: "var(--font-bricolage), 'Bricolage Grotesque', sans-serif" }}>{slot.icon}</p>
                    <p style={{ fontSize: 12, fontWeight: 600, color: slot.active ? "hsl(var(--foreground))" : "hsl(var(--muted-foreground))" }}>{slot.label}</p>
                    <p style={{ fontSize: 10, color: "hsl(var(--muted-foreground))" }}>{slot.sub}</p>
                  </div>
                ))}
              </div>
            </VisualInsightCard>
            <VisualInsightCard icon={<MapPin size={16} />} title="Location flexibility" question="How far are you willing to travel?">
              <div style={{ marginBottom: 8 }}>
                {[{ label: "Close to home / work", dist: "1.5 mi", idx: 0 }, { label: "Travel a bit", dist: "5 mi", idx: 1 }, { label: "Travel further for right clinic", dist: "5+ mi", idx: 2 }].map((loc) => (
                  <div key={loc.label} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 10, marginBottom: 4, background: loc.idx === 1 ? `hsla(var(--primary),.08)` : "transparent", border: loc.idx === 1 ? `1px solid hsla(var(--primary),.15)` : "1px solid transparent" }}>
                    <MapPin size={13} style={{ color: loc.idx === 1 ? teal : "hsl(var(--muted-foreground))", flexShrink: 0 }} />
                    <span style={{ fontSize: 12, color: loc.idx === 1 ? "hsl(var(--foreground))" : "hsl(var(--muted-foreground))", flex: 1, fontWeight: loc.idx === 1 ? 600 : 400 }}>{loc.label}</span>
                    <span style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", fontWeight: 500, background: "rgba(0,0,0,.05)", padding: "2px 8px", borderRadius: 6 }}>{loc.dist}</span>
                  </div>
                ))}
              </div>
            </VisualInsightCard>
          ...
          <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 18, overflow: "hidden", marginTop: 48 }}>
            <div style={{ overflowX: "auto" }}>
              <table className="fc-table" style={{ width: "100%", minWidth: 500, borderCollapse: "collapse", fontSize: 15 }}>
                <thead><tr>
                  <th style={{ textAlign: "left", padding: "18px 20px", color: "hsl(var(--muted-foreground))", fontWeight: 600, fontSize: 13, borderBottom: `1px solid rgba(0,0,0,.08)` }}>&nbsp;</th>
                  ...
                  <th style={{ textAlign: "center", padding: "18px 20px", color: teal, fontWeight: 700, fontSize: 14, borderBottom: `2px solid ${teal}`, background: `hsla(var(--primary),.04)` }}>Pearlie</th>
                </tr></thead>
                <tbody>
                  {[
                    ...
                    <tr key={row.feature}>
                      <td style={{ padding: "16px 20px", color: "hsl(var(--foreground))", fontWeight: 500, borderBottom: `1px solid rgba(0,0,0,.04)` }}>{row.feature}</td>
                      ...
                      <td style={{ textAlign: "center", padding: "16px 20px", color: teal, fontWeight: 600, fontSize: 14, borderBottom: `1px solid rgba(0,0,0,.04)`, background: `hsla(var(--primary),.03)` }}>{row.pearlie}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <section className="fc-section" style={sec}>
        <div style={{ ...maxW, maxWidth: 800 }}>
          <div style={{ textAlign: "center", marginBottom: 12 }}>
            <span style={{ display: "inline-block", background: `hsla(var(--primary),.1)`, color: teal, fontWeight: 700, fontSize: 13, padding: "6px 16px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.04em" }}>Zero risk</span>
          </div>
          {sectionH2(<>You only pay for patients<br />who actually show up</>)}
          <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 16, textAlign: "center", maxWidth: 600, margin: "0 auto 40px", lineHeight: 1.65 }}>
          ...
          <div className="fc-zero-risk-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            {[{ q: "Patient no-show?", a: "No charge. Ever." }, { q: "No patients this month?", a: "You don't pay extra." }, { q: "Want to cancel?", a: "Anytime. No penalties." }, { q: "Unhappy with quality?", a: "We'll make it right." }].map((card) => (
              <div key={card.q} style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 14, padding: "24px 22px" }}>
                <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 14, marginBottom: 8 }}>{card.q}</p>
      ...
      <section className="fc-section" style={secAlt}>
        <div style={maxW}>
          <div style={{ textAlign: "center", marginBottom: 12 }}>
            <span style={{ display: "inline-block", background: `hsla(var(--primary),.1)`, color: teal, fontWeight: 700, fontSize: 13, padding: "6px 16px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.04em" }}>Bonus: free visibility</span>
          </div>
          ...
          <div className="fc-visibility-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
            ...
            <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, padding: "24px 22px 20px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                ...
                <span style={{ fontWeight: 700, color: "hsl(var(--foreground))", fontSize: 14 }}>SEO-optimised profile</span>
              </div>
              <div style={{ background: "rgba(0,0,0,.04)", borderRadius: 10, padding: "14px 16px", marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}><div style={{ width: 14, height: 14, borderRadius: 3, background: "#4285f4" }} /><span style={{ color: "hsl(var(--muted-foreground))", fontSize: 11 }}>pearlie.org &rsaquo; clinic</span></div>
                <p style={{ color: "#60a5fa", fontSize: 13, fontWeight: 600, marginBottom: 4 }}>Your Clinic Name — Pearlie</p>
                <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, lineHeight: 1.4 }}>Invisalign, veneers &amp; cosmetic dentistry in London. View treatments, reviews &amp; book...</p>
              </div>
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, lineHeight: 1.4 }}>Rank on Google for local dental searches without paying for ads</p>
            </div>
            {/* Brand exposure */}
            <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, padding: "24px 22px 20px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                ...
                <span style={{ fontWeight: 700, color: "hsl(var(--foreground))", fontSize: 14 }}>Passive brand exposure</span>
              </div>
              {[{ label: "Profile views", value: "1,200+", pct: 80 }, { label: "Direct website clicks", value: "340+", pct: 45 }, { label: "Phone calls from profile", value: "85+", pct: 25 }].map((stat) => (
                <div key={stat.label} style={{ marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><span style={{ color: "hsl(var(--muted-foreground))", fontSize: 12 }}>{stat.label}</span><span style={{ color: "#a78bfa", fontSize: 12, fontWeight: 600 }}>{stat.value}/mo</span></div>
              ...
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, lineHeight: 1.4 }}>Patients see your clinic, learn about you — and may contact you directly</p>
            </div>
            {/* Social proof */}
            <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, padding: "24px 22px 20px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                ...
                <span style={{ fontWeight: 700, color: "hsl(var(--foreground))", fontSize: 14 }}>Social proof displayed</span>
              </div>
              <div style={{ background: "rgba(0,0,0,.04)", borderRadius: 10, padding: "14px 16px", marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                  {[1, 2, 3, 4, 5].map((s) => (<Star key={s} size={14} fill={s <= 4 ? "#d97706" : "none"} style={{ color: s <= 4 ? "#d97706" : "hsl(var(--muted-foreground))" }} />))}
                  <span style={{ color: "#d97706", fontSize: 13, fontWeight: 700, marginLeft: 4 }}>4.8</span><span style={{ color: "hsl(var(--muted-foreground))", fontSize: 11 }}>(127 reviews)</span>
                </div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              ...
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, lineHeight: 1.4 }}>Reviews, services &amp; specialties showcased to high-intent patients</p>
            </div>
            {/* Organic growth */}
            <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, padding: "24px 22px 20px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                <div style={{ width: 34, height: 34, borderRadius: 10, background: `hsla(var(--primary),.1)`, display: "flex", alignItems: "center", justifyContent: "center", color: teal, flexShrink: 0 }}><TrendingUp size={16} /></div>
                <span style={{ fontWeight: 700, color: "hsl(var(--foreground))", fontSize: 14 }}>Organic growth channel</span>
              </div>
              <div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 80, padding: "0 4px", marginBottom: 12 }}>
                {[18, 25, 22, 35, 40, 38, 52, 60, 55, 70, 78, 85].map((h, i) => (
                  <div key={i} style={{ flex: 1, height: `${h}%`, borderRadius: "3px 3px 0 0", background: i >= 9 ? `linear-gradient(180deg, ${teal}, hsla(var(--primary),.4))` : `hsla(var(--primary),.15)` }} />
                ))}
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}><span style={{ color: "hsl(var(--muted-foreground))", fontSize: 10 }}>Month 1</span><span style={{ color: teal, fontSize: 10, fontWeight: 600 }}>Month 12</span></div>
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, lineHeight: 1.4 }}>Even patients who don't book through Pearlie now know you exist</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div style={maxW}>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 12 }}>
              ...
              <span style={{ ...heading, fontSize: 20, color: "hsl(var(--foreground))" }}>Start free — &pound;0 upfront</span>
            </div>
            <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 15, lineHeight: 1.6, maxWidth: 520, margin: "0 auto 16px" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 20, marginBottom: 20 }}>
              {["No credit card required", "No contract", "Cancel anytime"].map((item) => (
                <div key={item} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span style={{ color: "hsl(var(--foreground))", fontSize: 13, fontWeight: 500 }}>{item}</span>
                </div>
              ))}
            </div>
            <Link
          ...
          >
            ...
            >
              <p style={{ color: "hsl(var(--muted-foreground))", fontWeight: 600, fontSize: 14, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.04em" }}>
                Starter
              </p>
              <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 6 }}>
                <span style={{ ...heading, fontSize: 48 }}>&pound;99</span>
                <span style={{ color: "hsl(var(--muted-foreground))", fontSize: 16 }}>/month</span>
              </div>
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 13, marginBottom: 24 }}>
                For single practices getting started
              </p>
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 14, marginBottom: 6 }}>
              ...
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, textAlign: "center", marginTop: 8 }}>
            ...
            <div
              style={{
                background: "hsla(var(--primary),.06)",
                border: "2px solid hsla(var(--primary),.25)",
                borderRadius: 20,
                padding: "40px 28px",
                position: "relative",
                display: "flex",
          ...
          <div
            style={{
              ...
              margin: "40px auto 0",
              background: "hsla(var(--primary),.06)",
              border: "1px solid hsla(var(--primary),.15)",
              borderRadius: 14,
              padding: "20px 28px",
              textAlign: "center",
            }}
          >
            <p style={{ color: "hsl(var(--foreground))", fontSize: 15, fontWeight: 600, lineHeight: 1.5 }}>
            ...
            <table className="fc-compare-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(0,0,0,.08)" }}>
                  <th style={{ textAlign: "left", padding: "16px 20px", color: "hsl(var(--muted-foreground))", fontWeight: 600, fontSize: 13, width: "40%" }}>&nbsp;</th>
                  <th style={{ textAlign: "center", padding: "16px 16px", color: "hsl(var(--foreground))", fontWeight: 700, fontSize: 14 }}>
                    Starter &pound;99
                  </th>
                  <th style={{ textAlign: "center", padding: "16px 16px", color: teal, fontWeight: 700, fontSize: 14, background: "hsla(var(--primary),.04)" }}>
              ...
              <tbody>
                ...
                <tr style={{ background: "hsla(var(--primary),.06)" }}>
                  <td style={{ padding: "14px 20px", color: "hsl(var(--foreground))", fontWeight: 700, fontSize: 14, borderBottom: "1px solid rgba(0,0,0,.04)" }}>
                  ...
                  <td style={{ textAlign: "center", padding: "14px 16px", borderBottom: "1px solid rgba(0,0,0,.04)", background: "hsla(var(--primary),.04)" }}><Check size={18} style={{ color: teal }} /></td>
                  <td style={{ textAlign: "center", padding: "14px 16px", borderBottom: "1px solid rgba(0,0,0,.04)" }}><Check size={18} style={{ color: teal }} /></td>
                </tr>
                {[
                  ...
                  <tr key={row.feature} style={{ borderBottom: "1px solid rgba(0,0,0,.04)" }}>
                    <td style={{ padding: "12px 20px", color: "hsl(var(--foreground))", fontWeight: 500 }}>{row.feature}</td>
                    {[row.starter, row.standard, row.premium].map((val, i) => (
                      <td key={i} style={{ textAlign: "center", padding: "12px 16px", background: i === 1 ? "hsla(var(--primary),.04)" : undefined }}>
                        {typeof val === "boolean" ? (val ? <Check size={16} style={{ color: teal }} /> : <X size={16} style={{ color: "hsl(var(--border))" }} />) : <span style={{ color: "hsl(var(--muted-foreground))", fontWeight: 600 }}>{val}</span>}
                      </td>
                    ))}
                  </tr>
                ))}
                ...
                {[
                  ...
                  <tr key={row.feature} style={{ borderBottom: "1px solid rgba(0,0,0,.04)" }}>
                    ...
                    {[row.starter, row.standard, row.premium].map((val, i) => (
                      <td key={i} style={{ textAlign: "center", padding: "12px 16px", background: i === 1 ? "hsla(var(--primary),.04)" : undefined }}>
                        {val ? <Check size={16} style={{ color: teal }} /> : <X size={16} style={{ color: "hsl(var(--border))" }} />}
                      </td>
                    ))}
                  </tr>
                ))}
                ...
                {[
                  ...
                  <tr key={row.feature} style={{ borderBottom: "1px solid rgba(0,0,0,.04)" }}>
                    <td style={{ padding: "12px 20px", color: row.feature === "Pearlie Guarantee" ? "#d97706" : "hsl(var(--foreground))", fontWeight: row.feature === "Pearlie Guarantee" ? 700 : 500 }}>
                ...
                <tr style={{ borderTop: "1px solid rgba(0,0,0,.08)" }}>
                  ...
                  <td style={{ textAlign: "center", padding: "20px 12px", background: "hsla(var(--primary),.04)" }}>
                    <Link href="/signup" style={{ ...greenBtn, padding: "10px 20px", fontSize: 13, borderRadius: 10 }}>
                      Start free trial
                    </Link>
                  </td>
      ...
      <section className="fc-section" style={sec}>
        <div style={{ ...maxW, maxWidth: 840 }}>
          ...
          <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 17, textAlign: "center", marginBottom: 48, lineHeight: 1.6 }}>
          ...
          <div className="fc-roi-wrapper" style={{ background: "rgba(0,0,0,.03)", border: "1px solid rgba(0,0,0,.06)", borderRadius: 20, padding: "36px 32px" }}>
            ...
            <div style={{ marginBottom: 40 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                <span style={{ color: "hsl(var(--muted-foreground))", fontSize: 14, fontWeight: 500 }}>Confirmed patients per month</span>
              ...
              <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, marginTop: 6 }}>0 = no subscription</p>
            </div>

            {/* LTV slider */}
            <div style={{ marginBottom: 40 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                <span style={{ color: "hsl(var(--muted-foreground))", fontSize: 14, fontWeight: 500 }}>Patient lifetime value</span>
                ...
                <>
                  <div className="fc-roi-output-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
                    <div style={{ background: "rgba(0,0,0,.04)", borderRadius: 14, padding: "20px 16px", textAlign: "center" }}>
                      <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 6 }}>You pay Pearlie</p>
                      <p style={{ ...heading, fontSize: 24, color: "hsl(var(--foreground))" }}>&pound;{r.totalCost.toLocaleString()}<span style={{ fontSize: 13, color: "hsl(var(--muted-foreground))", fontWeight: 400 }}>/mo</span></p>
                    </div>
                    <div style={{ background: "hsla(var(--primary),.06)", border: "1px solid hsla(var(--primary),.15)", borderRadius: 14, padding: "20px 16px", textAlign: "center" }}>
                      <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 6 }}>Revenue you keep</p>
                      <p style={{ ...heading, fontSize: 24, color: teal }}>&pound;{r.revenue.toLocaleString()}</p>
                    </div>
                    <div style={{ background: "rgba(0,0,0,.04)", borderRadius: 14, padding: "20px 16px", textAlign: "center" }}>
                      <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 6 }}>ROI</p>
                      <p style={{ ...heading, fontSize: 24, color: tealLight }}>{r.roiPct.toLocaleString()}%</p>
                    </div>
                    <div style={{ background: "rgba(0,0,0,.04)", borderRadius: 14, padding: "20px 16px", textAlign: "center" }}>
                      <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 6 }}>Net profit</p>
                      <p style={{ ...heading, fontSize: 24, color: "hsl(var(--foreground))" }}>&pound;{r.net.toLocaleString()}</p>
                    </div>
                  </div>
                  <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 14, textAlign: "center", lineHeight: 1.5 }}>
          ...
          <div className="fc-steps-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
            {[
              ...
              <div key={s.step} style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, padding: "28px 22px", textAlign: "center" }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: `hsla(var(--primary),.1)`, display: "flex", alignItems: "center", justifyContent: "center", color: teal, margin: "0 auto 14px" }}>{s.icon}</div>
                <p style={{ color: teal, fontSize: 11, fontWeight: 800, marginBottom: 6, letterSpacing: "0.06em" }}>STEP {s.step}</p>
                <h3 style={{ color: "hsl(var(--foreground))", fontWeight: 700, fontSize: 16, marginBottom: 6 }}>{s.title}</h3>
                <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 13, lineHeight: 1.45 }}>{s.desc}</p>
              </div>
            ))}
          </div>
          <p style={{ textAlign: "center", color: "hsl(var(--muted-foreground))", fontSize: 13, marginTop: 20 }}>Average time from signup to first matched patient: <span style={{ color: teal, fontWeight: 700 }}>&lt; 72 hours</span></p>
        </div>
      </section>
          ...
          <div className="fc-testimonials-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24, maxWidth: 880, margin: "48px auto 0" }}>
            {[
              ...
              <div key={t.name} style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 20, padding: "36px 30px", position: "relative" }}>
                <Quote size={28} style={{ color: `hsla(var(--primary),.1)`, position: "absolute", top: 28, right: 28 }} />
                <div style={{ display: "flex", gap: 4, marginBottom: 20 }}>{[1, 2, 3, 4, 5].map((s) => (<Star key={s} size={14} fill="#d97706" style={{ color: "#d97706" }} />))}</div>
                <p style={{ color: "hsl(var(--foreground))", fontSize: 15, lineHeight: 1.7, marginBottom: 28, fontStyle: "italic" }}>&ldquo;{t.quote}&rdquo;</p>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div><p style={{ color: "hsl(var(--foreground))", fontWeight: 700, fontSize: 14 }}>{t.name}</p><p style={{ color: "hsl(var(--muted-foreground))", fontSize: 12 }}>{t.practice}</p></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="guarantee" className="fc-section" style={sec}>
        <div style={{ ...maxW, maxWidth: 800 }}>
          ...
          <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 17, textAlign: "center", maxWidth: 560, margin: "0 auto 48px", lineHeight: 1.6 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 40 }} className="fc-zero-risk-grid">
            {[
              ...
              <div key={card.title} style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 16, padding: "28px 24px", boxShadow: "0 1px 4px rgba(0,0,0,.06)" }}>
                ...
                <h3 style={{ fontWeight: 700, color: "hsl(var(--foreground))", fontSize: 16, marginBottom: 8 }}>{card.title}</h3>
                <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 14, lineHeight: 1.55 }}>{card.desc}</p>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: "rgba(217,119,6,.06)", border: "1px solid rgba(217,119,6,.15)", borderRadius: 16, padding: "28px 32px", textAlign: "center" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 12 }}>
              ...
              <span style={{ ...heading, fontSize: 18, color: "hsl(var(--foreground))" }}>Why patients prefer clinics with the Guarantee</span>
            </div>
            <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 15, lineHeight: 1.6, maxWidth: 560, margin: "0 auto 16px" }}>
            ...
            <p style={{ color: "hsl(var(--foreground))", fontSize: 14, fontWeight: 600, margin: "0 auto 20px" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 24 }}>
              {[
                ...
                <div key={stat.label} style={{ textAlign: "center", minWidth: 100 }}>
                  ...
                  <div style={{ color: "hsl(var(--muted-foreground))", fontSize: 12, marginTop: 2, fontWeight: 500 }}>{stat.label}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ══════════ 13. FINAL CTA ══════════ */}
      <section className="fc-section" style={{ padding: "100px 24px 140px", background: `linear-gradient(180deg, hsla(var(--primary),.04) 0%, rgba(8,13,27,0) 100%)`, textAlign: "center" }}>
        <div style={maxW}>
          ...
          <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 18, maxWidth: 520, margin: "0 auto 44px", lineHeight: 1.6 }}>Join Pearlie and start receiving pre-qualified patients — with the insights you need to close them.</p>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 10, marginTop: 20 }}>
            {["No contracts", "No setup fee", "Cancel anytime"].map((item) => (
              <span key={item} style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "hsla(var(--primary),.07)", border: "1px solid hsla(var(--primary),.2)", padding: "6px 14px", borderRadius: 20, color: "hsl(var(--primary))", fontSize: 13, fontWeight: 600 }}>
                <Check size={13} /> {item}
              </span>
            ))}
          </div>
      ...
      <footer style={{ borderTop: `1px solid ${cardBorder}`, padding: "32px 24px", textAlign: "center" }}>
        <p style={{ color: "hsl(var(--muted-foreground))", fontSize: 13 }}>&copy; {new Date().getFullYear()} Pearlie Health Ltd. All rights reserved.</p>
      </footer>
    </div>
  )
}
